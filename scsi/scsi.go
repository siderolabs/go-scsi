...
func NewDevice(name string) (device *Device, err error) {
	...
	if version < 30000 {
		return nil, fmt.Errorf("no sg driver found for device at %s", name)
	}
...
func (dvc *Device) Identify() (err error) {
	...
	if err = dvc.StandardInquiry(); err != nil {
		return fmt.Errorf("standard inquiry failed: %v", err)
	}

	// Page 0 inquiry.
	var pages []byte
	if pages, err = dvc.Page0Inquiry(); err != nil {
		return fmt.Errorf("page 0 inquiry failed: %v", err)
	}
	for _, page := range pages {
		if page == Page80 || page == Page83 {
			// Page 80 inquiry.
			if err = dvc.Page80Inquiry(); err != nil {
				return fmt.Errorf("page 80 inquiry failed: %v", err)
			}
		}
		if page == Page83 {
			// Page 83 inquiry.
			if err = dvc.Page83Inquiry(); err != nil {
				return fmt.Errorf("page 83 inquiry failed: %v", err)
			}
		}
	}
...
func (dvc *Device) Page0Inquiry() (pages []byte, err error) {
	...
	if buf[3] > InguiryBufLen {
		return nil, fmt.Errorf("%s: page 0 buffer too big", dvc.Kernel)
	}
...
func (dvc *Device) Page80Inquiry() (err error) {
	...
	if len > MaxSerialLen {
		return fmt.Errorf("%d is larger than %d", len, MaxSerialLen)
	}
...
func (dvc *Device) inquiry(evpd, page uint8) (buf CommandDescriptorBlock, err error) {
	...
	if evpd == 1 && buf[1] != page {
		return buf, fmt.Errorf("%s: invalid VPD page: %x", dvc.Kernel, page)
	}
